...
setGeneric("getHeaderText",function(object,delim="\t")standardGeneric("getHeaderText"))  # Generic for retrieving RefData string from Objects
...
setMethod("head","refSeqDict",function(x,n=6L,...) {
  ...
  if (n < 0L)
    ...
    n<-min(n, dim(x@df)[1])
  if(n == 0L)
    return(x@df[integer(0),])
  else
    return(x@df[1:n,])
})
# S3 Generic is supplied via imporFrom in NAMESPACE
...
setGeneric("cigarSize",function(object) standardGeneric("cigarSize"))
setMethod(f="cigarSize",signature="bamAlign",definition=function(object) {return(.Call("bam_align_get_cigar_size",object@align,PACKAGE="rbamtools"))})
